...
class Regime:
    ...
    def decay(self, state: OperationalState, time: timedelta) -> tuple[OperationalState, BurnResult]:
        ...
        if power := state.power_nuc:
            raise ValueError(f"decay step is illegal because {power=}")
        if time < timedelta(0):
    ...
    def burn_to_pcm(
        ...
        *,
        ...
        maxstep: Optional[timedelta] = None,
        **oracle_kwargs,
    ) -> tuple[OperationalState, BurnResult]:
...
def reaction_rates_calculator(
    state: OperationalState,
    direct: callable,
    burnup_queries_factory: Callable[[OperationalState], Sequence[VolumeQuery]],
    **oracle_kwargs,
) -> [KResult, dict[PurePath, dict[Reaction, ReactionRate]]]:
