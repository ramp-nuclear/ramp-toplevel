...
def _mesh(
    lower_left: np.ndarray, upper_right: np.ndarray, resolution: tuple[cm, cm, cm]
...
def cartesian_flux_map(
    ...
    **kwargs,
    ...
    meshresult: MeshResult = calculator(state, query)[query][0]
...
def split_core(
    core: Core, components: tuple[PurePath, ...], resolution: tuple[cm, cm, cm]
) -> tuple[Core, dict[PurePath, tuple[PurePath, ...]]]:
    """
...
def _interpret_flux_map(core, results: Sequence[VolumeResult]) -> xr.Dataset:
    centers = tuple(path_centers(core, map(attrgetter("component"), results)))
    ...
    flux_map_df = pd.DataFrame(
        list(zip(x, y, z, energies, means, stds)),
        columns=["x", "y", "z", "e", "mean", "std"],
    ).set_index(["e", "x", "y", "z"])
    ...
    **kwargs,
    ...
    match method:
        case "mesh":
            ...
            # core model.
            bounding_boxes = (
                node.geometry.transform(core.transform_of(component)).bounding_box()
            ...
            results = valmap(lambda x: x[0].array, calculator(state, *queries.values()))
            return {component: results[queries[component]] for component in components}
        ...
        case "split":
            ...
            results = {
                component: list(
                    filter(
                        compose(partial(contains, children), attrgetter("component")),
                        results,
                    )
                )
