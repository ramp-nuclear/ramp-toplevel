...
def divide_periods(periods: Iterable[float], resolutions: Iterable[float]) -> Sequence[timedelta]:
    ...
    """
    return list(chain.from_iterable(starmap(_divide_period, zip(periods, resolutions))))


def midcycle_states(
    state: OperationalState,
    regime: Regime,
    times: Sequence[timedelta],
    saver: Callable[[str, OperationalState], None],
    initially_critical: bool = True,
    save_boc: bool = True,
    t0: timedelta = timedelta(0.0),
    **oracle_kwargs,
) -> OperationalState:
    """
    ...
    oracle_kwargs - keyword arguments that are supplied to the Regime.burnstep
        method.
    """
...
def critical_heights_temp_xenon_variations(
    state: OperationalState,
    ...
    """
    HFP_height = state.params[alias]
    guesses = np.linspace(HFP_height - height_search_length, HFP_height, guess_num)
