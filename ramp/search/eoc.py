...
def find_eoc_from_boc(state: OperationalState, *,
                      rho: PCM,
                      regime: Regime,
                      at_eoc: Callable[[KResult, PCM], bool],
                      too_risky: Callable[[KResult, PCM], bool],
                      max_safe_step: Callable) -> tuple[OperationalState, OperationalState]:
    ...
    if kwild.reactivity < rho and not at_eoc(kwild, rho):
        raise OperationalError("Could not burn the core for the required steps "
                               "and without going under the desired reactivity:"
                               f" {kwild.reactivity} < {rho}")
    return state, _find_eoc(state=state,
                            rho=rho,
                            regime=regime,
                            at_eoc=at_eoc,
                            too_risky=too_risky,
                            max_safe_step=max_safe_step,
                            drhodt=info.drhodt or -100.)


def _do_first_steps(regime: Regime, state: OperationalState) -> \
...
def _next_state(state: OperationalState, *,
    ...
    if forward:
        ...
        maxstep = max_safe_step(op_max_timestep=regime.maximal_timestep,
                                wanted_timestep=guess,
                                kres=kwild,
                                drhodt=drhodt,
                                rho_target=rho)
        state, info = regime.burn_to_pcm(state, rho=rho, guess=guess,
                                         maxstep=maxstep)
        logger.info(f'Burned for {info.time}, \n'
...
def _find_eoc(state: OperationalState, *,
              rho: PCM,
              drhodt: PCMPerSecond,
              regime: Regime,
              at_eoc: Callable[[KResult, PCM], bool],
              too_risky: Callable[[KResult, PCM], bool],
              max_safe_step: Callable) -> OperationalState:
    safe_state = state
    safe_reactivity = None
    _new_state = partial(_next_state, rho=rho, too_risky=too_risky,
                         regime=regime, max_safe_step=max_safe_step)
    kwild = regime.get_kwild(state)
    while not at_eoc(kwild, rho):
        state, safe_state, drhodt, safe_reactivity = _new_state(state, kwild=kwild,
                                                                safe=safe_state,
                                                                drhodt=drhodt,
...
def at_eoc_one_sigma(kres: KResult, rho: PCM, drho: PCM) -> bool:
    ...
    """
    return bool(np.isclose(kres.reactivity, rho,
                           atol=drho + kres.reactivity_error))
...
def max_safe_step_at_risk(*, op_max_timestep: timedelta,
                          wanted_timestep: timedelta,
                          kres: KResult,
                          drhodt: PCMPerSecond,
                          rho_target: PCM,
                          alpha: float = 1e-8,
                          minimal_timestep: timedelta = timedelta(days=1.)
                          ) -> timedelta:
    ...
    dist = NormalDist(0, kres.reactivity_error * sqrt(2.))
