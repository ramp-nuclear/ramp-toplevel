...
def find_height_at_given_reactivity(oracle: Oracle, state: OperationalState,
    ...
    scoring = partial(_scoring, func=oracle.direct,
                      given_reactivity=given_reactivity)
    # noinspection PyTypeChecker
    return best_choice(factory, scoring, guesses)


def find_critical_height(oracle: Oracle, state: OperationalState, alias,
                         guesses) -> Tuple[float, OperationalState, KResult]:
    ...
    """
    return find_height_at_given_reactivity(oracle=oracle,
                                           state=state,
                                           alias=alias,
                                           guesses=guesses,
                                           given_reactivity=0.0)


def _scoring(x: OperationalState, func: Callable,
             given_reactivity) -> Tuple[float, KResult]:
    query = KQuery()
