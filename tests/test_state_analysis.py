...
from coreoperator.mobilization import LoadChain, LoadSite, Scheme, Remove
...
def _to_oracle_result(reactivity, reactivity_error):
    reactivity = (
        reactivity if isinstance(reactivity, (float, int)) else reactivity.item()
    )
    reactivity_error = (
        reactivity_error
        if isinstance(reactivity_error, float)
        else reactivity_error.item()
    )
...
def test_get_rods_extraction_worth():
    ...
    def _calculator(state, queries=None, prefix=""):
        aluminum_block_node = example_state.core[
            PurePath("A1") / PurePath("coolant") / PurePath("aluminum_block")
        ]
        num_of_aluminum_blocks = sum(
            node == aluminum_block_node for _, node in state.core.nodes
        )
    ...
    delayed_rods_worths = rods_extraction_worth(
        example_state, sites, calculator=_calculator
    )
    (rods_worth,) = dask.compute(delayed_rods_worths)
    assert list(rods_worth.keys()) == sites
    assert all(
        [np.isclose(rods_worth[site].nominal_value, -rod_worth) for site in sites]
    )


def _get_control_height(state, alias):
    heights = {
        path: state.core.transform_of(path).translation[2]
...
def _control_heights_based_calculator(
    state, nominal_state, worth_per_cm, queries=None, prefix=""
):
...
def test_scurve():
    ...
    def _calculator(state, queries=None, prefix=""):
        return _control_heights_based_calculator(
            state, example_state, worth_per_cm, queries=queries, prefix=prefix
        )

    heights = np.linspace(-30.0, 30.0, 20)
    delayed_scurve_results = s_curve(
        example_state, "All", heights, calculator=_calculator
    )
    (scurve_results,) = dask.compute(delayed_scurve_results)
    assert np.allclose(heights, np.fromiter(scurve_results.keys(), dtype=float))
    reactivities = np.fromiter(
        (map(nominal_value, scurve_results.values())), dtype=float
    )
...
def test_decompose_action(factory):
    ...
    assert all(
        isinstance(action, Remove) | isinstance(action, LoadSite)
        for action in decomposed_action
    )
    sites_in_decomposed_action = [
        first(action.sites) if isinstance(action, Remove) else first(action.sites)[0]
        for action in decomposed_action
    ]
...
def test_stepwise_shuffle_reactivity(factory):
    ...
    (reactivities,) = dask.compute(
        stepwise_shuffle_reactivity(example_state, scheme, calculator=_calculator)
    )
