...
from coreoperator.mobilization import LoadChain, LoadSite, Remove, Scheme
from dask.delayed import Delayed
from more_itertools import first
from toolz import identity
from uncertainties.core import AffineScalarFunc, nominal_value

from ramp.state_analysis.control_rod_worth import moveable_reactivity_worth, s_curve
from ramp.state_analysis.rod_worths import rods_extraction_worth
from ramp.state_analysis.shuffle_worths import (
    decompose_action,
...
def test_get_rods_extraction_worth():
    ...
    def _calculator(state, queries=None, prefix=""):
        ...
        num_of_aluminum_blocks = sum(node == aluminum_block_node for _, node in state.core.nodes)
        nominal_num_of_aluminum_blocks = np.prod(example_state.core.grid.lattice.shape)
        reactivity = rod_worth * (num_of_aluminum_blocks - nominal_num_of_aluminum_blocks)
    ...
    delayed_rods_worths = rods_extraction_worth(example_state, sites, calculator=_calculator)
    (rods_worth,) = dask.compute(delayed_rods_worths)
    assert list(rods_worth.keys()) == sites
    assert all([np.isclose(rods_worth[site].nominal_value, -rod_worth) for site in sites])


def _get_control_height(state, alias):
    heights = {path: state.core.transform_of(path).translation[2] for path in state.core.aliases[alias][1]}
...
def _control_heights_based_calculator(state, nominal_state, worth_per_cm, queries=None, prefix=""):
...
def test_scurve():
    ...
    def _calculator(state, queries=None, prefix=""):
        return _control_heights_based_calculator(state, example_state, worth_per_cm, queries=queries, prefix=prefix)

    heights = np.linspace(-30.0, 30.0, 20)
    delayed_scurve_results = s_curve(example_state, "All", heights, calculator=_calculator)
    (scurve_results,) = dask.compute(delayed_scurve_results)
    assert np.allclose(heights, np.fromiter(scurve_results.keys(), dtype=float))
    reactivities = np.fromiter((map(nominal_value, scurve_results.values())), dtype=float)
    ratio = np.diff(reactivities) / np.diff(heights)
    assert np.allclose(ratio, worth_per_cm, rtol=1e-4, atol=0.0)
...
def test_movable_reactivity_worth():
    ...
    inserted = -30.0
    extracted = 30.0
    delayed_moveable_worth = moveable_reactivity_worth(
        example_state, "All", inserted, extracted, calculator=_calculator
    )
    ...
    assert np.isclose(
        moveable_worth.nominal_value,
        worth_per_cm * (extracted - inserted),
        rtol=1e-6,
        atol=0.0,
    )
...
def test_decompose_action(factory):
    ...
    transforms = [Transform((0.0, 0.0, z)) for z in np.linspace(0.0, 100.0, len(sites))]
    load_chain = LoadChain(factory=factory, sites=list(zip(sites, transforms)))
    decomposed_action = decompose_action(load_chain, example_state)
    assert len(decomposed_action) == 2 * len(sites)
    assert all(isinstance(action, Remove) | isinstance(action, LoadSite) for action in decomposed_action)
    sites_in_decomposed_action = [
        first(action.sites) if isinstance(action, Remove) else first(action.sites)[0] for action in decomposed_action
    ]
    assert all(site in sites for site in sites_in_decomposed_action), [
        site for site in sites_in_decomposed_action if site not in sites
    ]
    counter = Counter(sites_in_decomposed_action)
    assert all(counter[site] == 2 for site in sites)
    assert all(
        first(action.sites)[1] == t
        for action, t in zip(
            filter(lambda x: isinstance(x, LoadSite), decomposed_action),
            reversed(transforms),
        )
    )
...
def test_stepwise_shuffle_reactivity(factory):
    ...
    load_chain = LoadChain(factory=factory, sites=sites)
    ...
    (reactivities,) = dask.compute(stepwise_shuffle_reactivity(example_state, scheme, calculator=_calculator))
